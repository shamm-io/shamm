          ...
          <div className="flex items-center justify-end gap-x-3 text-white whitespace-nowrap">
            ...
            {user ?
              <div className="relative">
                <div className="cursor-pointer flex items-center gap-x-2" onClick={handleOpen}>
                  <img className="rounded-full" height={30} width={30} src={user.photoURL}></img>
                  <span className="text-base text-money-green">{user.displayName}</span>
                  <svg className="w-12" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#1BC622" viewBox="0 0 16 16"> <path fillRule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z" fill="#1BC622"></path> </svg>
                </div>
                  {open ? <div className="absolute w-full top-10 text-left bg-nero py-2 px-5 border-2 shadow-card border-card-border rounded-lg">
                    <ul className="text-base leading-8">
                      <li className="hover:text-money-green hover:font-medium transition-all">My Projects</li>
                      <li className="hover:text-money-green hover:font-medium transition-all">Project Contributions</li>
                      <li className="hover:text-money-green hover:font-medium transition-all" onClick={() => auth.signOut()}>Logout</li>
                    </ul>
                  </div> : ''}
              </div>
              :
              <>
                <button className="font-medium text-sm border-2 py-1 px-3 border-white rounded-md" onClick={signIn}>
                  Sign up
                </button>
                <button className="font-medium text-sm border-2 py-1 px-3 border-black bg-black rounded-md shammBtn" onClick={signIn}>
                  Login
                </button>
              </>
            }
